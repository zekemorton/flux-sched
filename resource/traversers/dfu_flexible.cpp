...
int dfu_flexible_t::select (Jobspec::Jobspec &j, vtx_t root, jobmeta_t &meta, bool excl)
{
    ...
    for (auto &resources : xor_resources) {
        if (select_resources (root, resources, meta, excl) == 0) {
    ...
    const std::vector<Resource> &resources) const
{
    ...
    for (const auto &resource : resources) {
        ...
        if (resource.type == xor_slot_rt) {
            ...
            if (xor_options.size () > detail::SYSTEM_MAX_XOR_EXPANSION) {
                errno = EOVERFLOW;
                return {};
            }
        ...
        if (next_variants.size () > detail::SYSTEM_MAX_XOR_EXPANSION) {
            errno = EOVERFLOW;
            return {};
        }
    ...
    if (results.size () > detail::SYSTEM_MAX_XOR_EXPANSION) {
        errno = EOVERFLOW;
        return {};
    }
