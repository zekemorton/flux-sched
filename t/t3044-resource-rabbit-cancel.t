...
rabbit_rack_jgf="${SHARNESS_TEST_SRCDIR}/data/resource/jgfs/rabbit-rack.json"
ssd_collection_jgf="${SHARNESS_TEST_SRCDIR}/data/resource/jgfs/ssd-collection.json"
...
# would remain allocated to the released jobs, and neither of two
# subsequent jobs could allocate.
#

cmds007="${cmd_dir}/cmds03.in"
test007_desc="releasing two jobs across both chassis leaves no rabbit capacity allocated"
...
#
# Cases (k) and (l): general storage topologies.  The release path must
# not depend on the containment shape that rabbit systems happen to use
# today.  These two cases guard against regressions in topologies that
# are not specifically rabbits, but are more general shapes exemplified
# by potential rabbit configurations a system administrator could
# choose:
#   (k) storage hanging off a rack (rack -> {ssd, storage_node, node}),
#       the representation flux-coral2 used before chassis, and
#   (l) storage as a bare collection of ssd vertices, with no
#       rabbit/storage_node vertex at all (chassis -> {ssd, node}).
# Both fixtures derive from rabbit.json: (k) renames the chassis
# vertices to rack; (l) deletes the storage_node vertices and their
# cores.  Both cases run the case (a) flow: allocate, partial-cancel the
# job's two node ranks (the only ranks a production .free fragment can
# name), fully cancel, verify that no vertex is left tagged or
# allocated, and re-allocate the freed capacity.
#

cmds015="${cmd_dir}/cmds01.in"
test015_desc="partial-cancel + cancel does not leak rack-hung storage (general topology)"
test_expect_success "${test015_desc}" '
    sed "s~@TEST_SRCDIR@~${SHARNESS_TEST_SRCDIR}~g" ${cmds015} > cmds015 &&
    ${query} -f jgf -L ${rabbit_rack_jgf} -S CA -P first -t 015.R.out < cmds015 &&
    test_cmp 015.R.out ${exp_dir}/015.R.out
'

# Case (l) intentionally shares case (a)'s expected output: the
# storage_node vertex never joins the allocation, so its absence must
# not change the emitted R or the release behavior at all.

cmds016="${cmd_dir}/cmds01.in"
test016_desc="partial-cancel + cancel does not leak a bare ssd collection (general topology)"
test_expect_success "${test016_desc}" '
    sed "s~@TEST_SRCDIR@~${SHARNESS_TEST_SRCDIR}~g" ${cmds016} > cmds016 &&
    ${query} -f jgf -L ${ssd_collection_jgf} -S CA -P first -t 016.R.out < cmds016 &&
    test_cmp 016.R.out ${exp_dir}/001.R.out
'
